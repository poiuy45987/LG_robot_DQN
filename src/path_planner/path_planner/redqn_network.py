...
class CNN_ReDQN(nn.Module):
    def __init__(self, **kwargs):
        ...
        # kwargs 처리
        self.action_size = kwargs.get('action_size', 16)
        self.all_dir_num = kwargs.get('all_dir_num', 28)
        ...
        next_dim = get_next_layer_dim(self.local_view_dim+2, kernel_size=3, stride=1, padding=1) # 53 -> 53
        next_dim = get_next_layer_dim(next_dim, kernel_size=3, stride=2, padding=0) # 53 -> 26
        final_dim = get_next_layer_dim(next_dim, kernel_size=3, stride=2, padding=0) # 26 -> 12
        ...
        self.vec_encoder = nn.Sequential(
            nn.Linear(2+self.all_dir_num+self.action_size+1, 32),
            nn.ReLU(),
            nn.Linear(32, 64),
            nn.ReLU(),
        )
