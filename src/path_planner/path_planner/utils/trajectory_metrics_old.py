...
def _overlap_stats(
    ...
    *,
    ...
    # 현재 위치의 footprint와 자연스럽게 겹친 영역은 overlap에서 제외
    segments = [(waypoints_m[0], waypoints_m[0])] if len(waypoints_m) == 1 else list(zip(waypoints_m[:-1], waypoints_m[1:]))
    for p0, p1 in segments:
        swept = _segment_footprint_cells(
            p0,
            p1,
            resolution_m=resolution_m,
            robot_diameter_m=robot_diameter_m,
        )
        current_footprint = _footprint_cells(
            float(p0[0]),
            float(p0[1]),
            resolution_m=resolution_m,
            robot_diameter_m=robot_diameter_m,
        )
        # 현재 위치 footprint를 벗어난 영역을 다시 덮을 때만 overlap으로 계산
        total_overlap += len((swept - current_footprint) & swept_once)
        swept_once.update(swept)
