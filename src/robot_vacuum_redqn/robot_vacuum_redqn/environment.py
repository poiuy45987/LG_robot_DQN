...
class CoverageEnv(gym.Env):
    ...
    def __init__(self, cfg: EnvConfig, seed: int | None = None):
        ...
        self.active_trace_indices = None # trace map에서 현재 활성화된 좌표들의 indices를 담는 배열. Shape: (N, 2)
    ...
    def _get_traj_data(self, new_cleaned_num: int, new_covered_cell_indices: np.array) -> dict:
        ...
        """
        active_trace_value = self.trace[self.active_trace_indices[:, 1], self.active_trace_indices[:, 0]]
        trace_map_data = (self.active_trace_indices, active_trace_value) # 현재 활성화된 trace 좌표들과 그 값
    ...
    def one_step_back(self, remain_collision_count: bool = False):
        ...
        active_trace_indices, active_trace_value = last_instance_data['trace_map_data']
        ...
        # self.cfg.stack_steps 전의 trace layer 얻음
        idx = -self.cfg.stack_steps
        if len(self.traj) < self.cfg.stack_steps:
            idx = 0
        past_traj_data = self.traj[idx]
        past_trace = np.zeros((self.H, self.W), dtype=np.uint8)
