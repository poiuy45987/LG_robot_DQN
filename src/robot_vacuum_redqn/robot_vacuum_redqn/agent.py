...
class DQNAgent:
    ...
    def _pre_process_obs(self, obs, local_view_dim=51) -> dict:
        ...
        # cleaned map을 0, 1 형태로 변경
        cleaned_map_layer_num = np.arange(1, num_layers, 3) # cleaned map layer는 1, 4, 7, ... 번째 layer                    
        hwc_map[:, :, cleaned_map_layer_num] = (hwc_map[:, :, cleaned_map_layer_num] > 0).astype(np.float32)
        
        resized_hwc_map = np.zeros((local_view_dim, local_view_dim, num_layers), dtype=np.float32)
        
        do_resize = (hwc_map.shape[0] != local_view_dim) or (hwc_map.shape[1] != local_view_dim)
        
        if do_resize:
            for i in range(num_layers):
                # 중앙의 local_view를 resize
                resized_hwc_map[:, :, i] = cv2.resize(
                    hwc_map[:, :, i], 
                    (local_view_dim, local_view_dim), 
    ...
    def _validation(self, episode: int):
        ...
        if self.wandb_run:
            self.wandb_run.log({'Validation/Coverage_mean': coverage_mean,
                                'Validation/Overlap_rate_mean': overlap_rate_mean,
                                'Validation/Cleaning_time_mean': cleaning_time_mean,
